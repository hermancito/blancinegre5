<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\App\Model\Entity\Notifmonitor> $notifmonitors
 */
?>
<div class="notifmonitors index content">
    <?= $this->Html->link(__('New Notifmonitor'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Notifmonitors') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('user_id') ?></th>
                    <th><?= $this->Paginator->sort('nombre') ?></th>
                    <th><?= $this->Paginator->sort('photo') ?></th>
                    <th><?= $this->Paginator->sort('photo_dir') ?></th>
                    <th><?= $this->Paginator->sort('validado') ?></th>
                    <th><?= $this->Paginator->sort('comodin') ?></th>
                    <th><?= $this->Paginator->sort('tipo_monitor') ?></th>
                    <th><?= $this->Paginator->sort('created') ?></th>
                    <th><?= $this->Paginator->sort('modified') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($notifmonitors as $notifmonitor): ?>
                <tr>
                    <td><?= $this->Number->format($notifmonitor->id) ?></td>
                    <td><?= $notifmonitor->hasValue('user') ? $this->Html->link($notifmonitor->user->email, ['controller' => 'Users', 'action' => 'view', $notifmonitor->user->id]) : '' ?></td>
                    <td><?= h($notifmonitor->nombre) ?></td>
                    <td><?= h($notifmonitor->photo) ?></td>
                    <td><?= h($notifmonitor->photo_dir) ?></td>
                    <td><?= h($notifmonitor->validado) ?></td>
                    <td><?= h($notifmonitor->comodin) ?></td>
                    <td><?= h($notifmonitor->tipo_monitor) ?></td>
                    <td><?= h($notifmonitor->created) ?></td>
                    <td><?= h($notifmonitor->modified) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $notifmonitor->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $notifmonitor->id]) ?>
                        <?= $this->Form->postLink(
                            __('Delete'),
                            ['action' => 'delete', $notifmonitor->id],
                            [
                                'method' => 'delete',
                                'confirm' => __('Are you sure you want to delete # {0}?', $notifmonitor->id),
                            ]
                        ) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>