<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\App\Model\Entity\ActivitatsAlumno> $activitatsAlumnos
 */
?>
<div class="activitatsAlumnos index content">
    <?= $this->Html->link(__('New Activitats Alumno'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Activitats Alumnos') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('activitat_id') ?></th>
                    <th><?= $this->Paginator->sort('alumno_id') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($activitatsAlumnos as $activitatsAlumno): ?>
                <tr>
                    <td><?= $this->Number->format($activitatsAlumno->id) ?></td>
                    <td><?= $activitatsAlumno->hasValue('activitat') ? $this->Html->link($activitatsAlumno->activitat->nombre, ['controller' => 'Activitats', 'action' => 'view', $activitatsAlumno->activitat->id]) : '' ?></td>
                    <td><?= $activitatsAlumno->hasValue('alumno') ? $this->Html->link($activitatsAlumno->alumno->codigo, ['controller' => 'Alumnos', 'action' => 'view', $activitatsAlumno->alumno->id]) : '' ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $activitatsAlumno->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $activitatsAlumno->id]) ?>
                        <?= $this->Form->postLink(
                            __('Delete'),
                            ['action' => 'delete', $activitatsAlumno->id],
                            [
                                'method' => 'delete',
                                'confirm' => __('Are you sure you want to delete # {0}?', $activitatsAlumno->id),
                            ]
                        ) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>